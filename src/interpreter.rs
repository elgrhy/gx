...
impl Interpreter {
        ...
        if let Some(brain) = &helper.brain.clone() {
            ...
            loop {
                ...
                match self.run_brain(brain, &mut env) {
                    ...
                    Err(Signal::ReRun) => {
                        return Err(Signal::Error(format!("Helper '{}' exceeded {} re-run cycles", helper.name, MAX_CYCLES)));
                    }
                    Err(e) => return Err(e),
                }
                memory = env.get_memory();
        ...
        for wb in &helper.when_blocks.clone() {
            ...
            match &wb.trigger {
                ...
                WhenTrigger::Changes(expr) => {
                    ...
                    let key = format!("__prev_{:?}", expr).replace('"', "").replace(' ', "_");
    ...
    fn run_stmts(&mut self, stmts: &[Stmt], env: &mut Env) -> IResult {
        ...
        for stmt in stmts { last = self.run_stmt(stmt, env)?; }
        Ok(last)
    }
    ...
    fn run_stmt(&mut self, stmt: &Stmt, env: &mut Env) -> IResult {
        match stmt {
            ...
            Stmt::If { branches, else_body, .. } => {
                ...
                if let Some(body) = else_body { return self.run_stmts(body, env); }
                Ok(Value::Null)
            }
            ...
            Stmt::Return { value, .. } => {
                let v = match value { Some(e) => self.eval_expr(e, env)?, None => Value::Null };
                Err(Signal::Return(v))
            }
            ...
            Expr::FieldAccess { object, field } => {
                match object.as_ref() {
                    Expr::Ident(obj_name) => {
                        ...
                        if matches!(obj, Value::Null) { obj = Value::Object(HashMap::new()); }
                    ...
                    Expr::FieldAccess { object: inner_obj, field: inner_field } => {
                        ...
                        if matches!(outer, Value::Null) { outer = Value::Object(HashMap::new()); }
                        let mut inner = outer.get_field(inner_field);
                        if matches!(inner, Value::Null) { inner = Value::Object(HashMap::new()); }
                ...
                if let Expr::Ident(name) = object.as_ref() {
                    ...
                    match (&mut obj, &idx) {
                        (Value::Array(arr), Value::Number(n)) => {
                            ...
                            else { return Err(Signal::Error(format!("Array index {} out of bounds", i))); }
                        }
                        (Value::Object(map), Value::Str(k)) => { map.insert(k.clone(), val); }
                        _ => return Err(Signal::Error("Cannot index assign to this type".into())),
                    }
                    env.set(name, obj);
                    Ok(())
                } else {
                    Err(Signal::Error("Cannot assign to complex index expression".into()))
                }
            }
    ...
    pub fn eval_expr(&mut self, expr: &Expr, env: &mut Env) -> IResult {
        match expr {
            ...
            Expr::Interpolated(parts) => {
                ...
                for part in parts {
                    match part {
                        ...
                        InterpolatedPart::Expr(e)    => s.push_str(&self.eval_expr(e, env)?.to_string()),
                    }
                }
                Ok(Value::Str(s))
            }
            ...
            Expr::Object(pairs) => {
                ...
                for (k, v) in pairs { map.insert(k.clone(), self.eval_expr(v, env)?); }
                Ok(Value::Object(map))
            }

            Expr::Array(items) => {
                let mut arr = Vec::new();
                for item in items { arr.push(self.eval_expr(item, env)?); }
                Ok(Value::Array(arr))
            }
            ...
            Expr::AskAI { provider, model, params } => {
            ...
            Expr::BridgeCall { namespace, module, method, args } => {
    ...
    fn append_ai_trace(&mut self, env: &mut Env, result: &Value) {
        ...
        let trace = memory.entry("ai_trace".into()).or_insert_with(|| Value::Array(Vec::new()));
    ...
    fn add_values(&self, lv: &Value, rv: &Value) -> IResult {
        match (lv, rv) {
            ...
            (Value::Array(a), Value::Array(b))   => {
                let mut arr = a.clone(); arr.extend(b.clone()); Ok(Value::Array(arr))
            }
    ...
    fn eval_builtin(&mut self, name: &str, args: Vec<Value>, _env: &mut Env) -> IResult {
        match name {
            ...
            "to_string" => Ok(Value::Str(args.first().cloned().unwrap_or(Value::Null).to_string())),
            "to_number" => {
                match args.first().cloned().unwrap_or(Value::Null) {
                    Value::Number(n) => Ok(Value::Number(n)),
                    Value::Str(s) => s.parse::<f64>().map(Value::Number)
                        .map_err(|_| Signal::Error(format!("Cannot convert '{}' to number", s))),
                    _ => Ok(Value::Number(0.0)),
                }
            }
            "type_of" => Ok(Value::Str(args.first().cloned().unwrap_or(Value::Null).type_name().into())),
            "is_null"  => Ok(Value::Bool(matches!(args.first(), Some(Value::Null) | None))),
            "keys" => {
                match args.first().cloned().unwrap_or(Value::Null) {
                    Value::Object(m) => Ok(Value::Array(m.keys().map(|k| Value::Str(k.clone())).collect())),
            ...
            "floor"  => Ok(Value::Number(args.first().and_then(|v| v.as_number()).unwrap_or(0.0).floor())),
            "ceil"   => Ok(Value::Number(args.first().and_then(|v| v.as_number()).unwrap_or(0.0).ceil())),
            "round"  => Ok(Value::Number(args.first().and_then(|v| v.as_number()).unwrap_or(0.0).round())),
            "abs"    => Ok(Value::Number(args.first().and_then(|v| v.as_number()).unwrap_or(0.0).abs())),
            "max"    => {
                let a = args.first().and_then(|v| v.as_number()).unwrap_or(f64::NEG_INFINITY);
                let b = args.get(1).and_then(|v| v.as_number()).unwrap_or(f64::NEG_INFINITY);
                Ok(Value::Number(a.max(b)))
            }
            "min"    => {
                let a = args.first().and_then(|v| v.as_number()).unwrap_or(f64::INFINITY);
                let b = args.get(1).and_then(|v| v.as_number()).unwrap_or(f64::INFINITY);
                Ok(Value::Number(a.min(b)))
            }
            // Stubs for runtime management (Phase 7)
            "spawn_agent" | "spawn_helper" => {
                let n = args.first().cloned().unwrap_or(Value::Str("unknown".into()));
                eprintln!("[gx] spawning agent: {}", n);
                Ok(Value::Str(format!("agent:{}", n)))
            }
            "wait_for_agent_ready" | "start_application" | "stop_application"
            | "restart_application" | "restart_all_failed_agents"
            | "initialize_memory_manager" | "initialize_message_router"
            | "initialize_helper_manager" | "parse_gx_file"
            | "execute_initial_brain_cycles" | "load_application"
            | "generate_job_id" | "start_training_process"
            | "monitor_training_jobs" | "deploy_ready_models"
            | "update_model_performance" | "cleanup_old_models" => Ok(Value::Null),
            _ => {
    ...
    fn eval_method(&mut self, obj: Value, method: &str, args: Vec<Value>, _env: &mut Env) -> IResult {
        match (&obj, method) {
            (Value::Array(arr), "push") => {
                let mut a = arr.clone();
                if let Some(v) = args.into_iter().next() { a.push(v); }
                Ok(Value::Array(a))
            }
            (Value::Array(arr), "pop") => {
                let mut a = arr.clone();
            ...
            (Value::Array(arr), "length") | (Value::Array(arr), "len") => Ok(Value::Number(arr.len() as f64)),
            ...
            (Value::Array(arr), "join")  => {
                let sep = args.first().and_then(|v| v.as_str().map(String::from)).unwrap_or_default();
                Ok(Value::Str(arr.iter().map(|v| v.to_string()).collect::<Vec<_>>().join(&sep)))
            }
            ...
            (Value::Array(arr), "reverse") => {
                let mut a = arr.clone(); a.reverse(); Ok(Value::Array(a))
            }
            ...
            (Value::Str(s), "split")      => {
                let sep = args.first().and_then(|v| v.as_str().map(String::from)).unwrap_or(" ".into());
                Ok(Value::Array(s.split(&*sep).map(|p| Value::Str(p.to_string())).collect()))
            }
            (Value::Str(s), "contains")   => {
                let needle = args.first().and_then(|v| v.as_str().map(String::from)).unwrap_or_default();
                Ok(Value::Bool(s.contains(&*needle)))
            }
            (Value::Str(s), "starts_with") => {
                let p = args.first().and_then(|v| v.as_str().map(String::from)).unwrap_or_default();
            ...
            (Value::Object(m), "keys")   => Ok(Value::Array(m.keys().map(|k| Value::Str(k.clone())).collect())),
...
impl Interpreter {
    pub fn call_js(&mut self, module: &str, method: &str, args: &[Value]) -> Result<Value, Signal> {
        ...
        use crate::bridge::{value_to_json, json_to_value};

        let json_args = serde_json::to_string(
            &args.iter().map(value_to_json).collect::<Vec<_>>()
        ...
        match serde_json::from_str::<serde_json::Value>(last_line) {
            Ok(json) => {
                if json["ok"].as_bool() == Some(true) {
                    ...
                    Err(Signal::Error(format!("JS error: {}", json["error"].as_str().unwrap_or("unknown"))))
                }
            }
...
impl Interpreter {
    pub fn bridge_call(&mut self, namespace: &str, module: &str, method: &str, args: &[Value]) -> Result<Value, Signal> {
        match namespace {
                ...
                let bridge = match self.py_bridge.as_mut() {
                    ...
                    None => {
                        match Bridge::new_python() {
                            Ok(b) => { self.py_bridge = Some(b); self.py_bridge.as_mut().unwrap() }
            ...
            "rust" => Err(Signal::Error(format!(
                "Native Rust interop for '{}' requires recompiling GX with the crate linked.", module
            ))),
            other => Err(Signal::Error(format!("Unknown namespace '{}'. Use: js, py, rust", other))),
        }
    }
}
